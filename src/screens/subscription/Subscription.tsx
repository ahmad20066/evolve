...
const Subscription = ({ navigation }: AppNavigationProps<"Subscription">) => {
  ...
  // Helper function to get subscription duration info
  const getSubscriptionDurationInfo = (subscriptionDuration?: number | null) => {
    ...
    if (subscriptionDuration === 21) {
      return {
        label: t("days_21"),
        excludedDays: [t("friday"), t("saturday")],
      };
    }
    if (subscriptionDuration === 26) {
      return {
        label: t("days_26"),
      ...
      ? [
          {
            ...
            price:
              data?.dietSubscription?.subscription_duration === 21
                ? data?.dietSubscription?.meal_plan?.price_21_days
                : data?.dietSubscription?.subscription_duration === 26
                ? data?.dietSubscription?.meal_plan?.price_26_days
                : data?.dietSubscription?.meal_plan?.price_monthly,
            ...
            subscriptionDuration: data?.dietSubscription?.subscription_duration,
          },
        ]
      : []),
  ...
  return (
    <View style={[globalStyles.container, styles.container]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {items?.map((item) => {
          const durationInfo =
            item.type === t("meal_plan")
              ? getSubscriptionDurationInfo(item.subscriptionDuration)
              : null;
            ...
            <View key={item.key}>
              ...
              <View key={item.key} style={styles.item}>
                ...
                <View style={styles.smallbox}>
                  ...
                  {durationInfo && (
                    <View style={[globalStyles.line, styles.durationRow]}>
                      <Calendar
                        width={14}
                        height={14}
                      ...
                      >
                        {durationInfo.label}
                        {durationInfo.excludedDays.length > 0 && (
                          <Text variant="poppins12black_regular" color="gray">
                            {" "}
                            ({t("excludes")}: {durationInfo.excludedDays.join(` ${t("and")} `)})
                          </Text>
                        )}
                        {durationInfo.excludedDays.length === 0 && (
                          <Text variant="poppins12black_regular" color="gray">
                ...
                <RNBounceable
                  ...
                  onPress={() =>
                    mutate({
                      id: item.key!,
                      type: item.type == t("meal_plan") ? "diet" : "fitness",
                    })
                  }
                >
                  {isPending ? (
